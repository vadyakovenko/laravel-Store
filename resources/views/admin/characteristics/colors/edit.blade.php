...
@section('content')
    <div class="box">
        <form method="POST" action="{{route('admin.characteristics.colors.update', $color)}}">
            @method('PUT')
            @csrf
            ...
            <div class="box-body">
                <div class="col-md-4">
                    <div class="form-group {{!$errors->has('name') ? : 'has-error'}}">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="{{ $color->name}}" placeholder="Название">
                        <span class="help-block">{{$errors->first('name')}}</span>
                    </div>
                    <div class="form-group {{ !$errors->has('value')?:'has-error' }}">
                        <label for="value" >Значение*:</label>
                        <span class="help-block">{{$errors->first('value')}}</span>
                        <input type="text" id="value" name='value' class="coloringpick" value="{{ $color->value }}">
                    </div>
                </div>
            </div>
            <!-- /.box-body -->
