...
class SeparatorField 
{
    public static function createField(string $type, array $arguments)
    {
        ...
        if(preg_match( '#separator_selector$#', $type)) {
            return SelectorSeparatorField::create(new Selector($arguments[$type . '_selector'], $arguments[$type . '_selector_type']));
        }

        if(preg_match( '#separator_substractor$#', $type)) {
            return SubstructorSeparatorField::create(new Selector($arguments[$type . '_main_selector'], $arguments[$type . '_main_selector_type']), new Selector($arguments[$type . '_cut_selector'], $arguments[$type . '_cut_selector_type']));
        }
